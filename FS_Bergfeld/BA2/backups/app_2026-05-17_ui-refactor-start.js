...
let spiderSessionOpen = false;
let spiderBatchChanges = [];
...
const mehrTab = document.querySelector("#mehrTab");
...
const mehrView = document.querySelector("#mehrView");
...
let liveHintText = document.querySelector("#liveHintText");
const liveSmartHints = document.querySelector("#liveSmartHints");
const liveSuggestions = document.querySelector("#liveSuggestions");
let manualItemSelect = document.querySelector("#manualItemSelect");
let saveLiveObservationBtn = document.querySelector("#saveLiveObservationBtn");
...
document.addEventListener("DOMContentLoaded", () => {
  liveText = liveText ?? document.querySelector("#liveText");
  liveHintText = liveHintText ?? document.querySelector("#liveHintText");
  manualItemSelect = manualItemSelect ?? document.querySelector("#manualItemSelect");
  saveLiveObservationBtn = saveLiveObservationBtn ?? document.querySelector("#saveLiveObservationBtn");
...
function bindEvents() {
  ...
  toggleChartBtn?.addEventListener("click", () => {
    if (!chartOverlayOpen) {
      spiderSessionOpen = true;
      spiderBatchChanges = [];
    } else {
      flushSpiderBatch();
      spiderSessionOpen = false;
    }
  ...
  mehrTab?.addEventListener("click", () => {
    currentView = "mehr";
    detailOpen = false;
    render();
  });
...
function processLocalRulesOnDirtyEntries() {
  const entries = session.rawObservations ?? [];
  let changed = false;
  for (const obs of entries) {
    if (obs.excluded || obs.analysisStatus !== "dirty") continue;
    const text = obs.spiderBatch
      ? (obs.spiderBatchItems ?? []).map((i) => i.text).join(" ")
      : (obs.text ?? "");
    const { autoLinks, generatedCondensations } = matchLocalRules(text);
    obs.autoLinks = autoLinks;
    obs.generatedCondensations = generatedCondensations;
    obs.analysisStatus = "processed";
    obs.lastAnalyzedAt = new Date().toISOString();
    changed = true;
  }
  if (changed) {
    session._hasDirtyEntries = false;
...
function scheduleDirtyCheck() {
  ...
  dirtyCheckTimer = window.setTimeout(() => {
    ...
    session._hasDirtyEntries = (session.rawObservations ?? []).some(
      (obs) => !obs.excluded && obs.analysisStatus === "dirty"
    );
    if (session._hasDirtyEntries) processLocalRulesOnDirtyEntries();
  }, 1500);
}
...
function saveRawObservation() {
  ...
  draft.analysisStatus = "dirty";
  draft.lastAnalyzedAt = null;
  draft.autoLinks = [];
  draft.generatedCondensations = [];
  draft.usedForInterpretation = true;
  session.rawObservations ??= [];
...
function renderRawObservationCard(observation) {
  if (observation.spiderBatch) {
    const pos = (observation.spiderBatchItems ?? []).filter((i) => i.valence === "positive");
    const dev = (observation.spiderBatchItems ?? []).filter((i) => i.valence === "development");
    const count = (observation.spiderBatchItems ?? []).length;
    const isSelected = expandedLiveEventId === observation.id;
    return `
      <article class="raw-observation spider-batch-card ${isSelected ? "selected" : ""}" data-live-id="${observation.id}" title="Spider-Web-Sammeleintrag">
        <button class="raw-relevant-btn${observation.gespraechsrelevant ? " active" : ""}" type="button" data-live-relevant="${observation.id}" aria-label="Gesprächsrelevant markieren">❗</button>
        <div class="raw-time">${formatTime(observation.timestamp)}</div>
        <div class="raw-main">
          <strong>Spider-Web-Markierung${observation.phase ? " · " + escapeHtml(observation.phase) : ""} · ${count} Item${count !== 1 ? "s" : ""}</strong>
          ${pos.length ? `<p class="spider-batch-line">Grün: ${escapeHtml(pos.map((i) => i.text).join(", "))}</p>` : ""}
          ${dev.length ? `<p class="spider-batch-line">Blau: ${escapeHtml(dev.map((i) => i.text).join(", "))}</p>` : ""}
        </div>
  ...
  if (observation.spiderBatch) {
    ...
    return `
      <div class="raw-inspector-header">
        <span>${formatTime(observation.timestamp)} · Spider-Web-Markierung${observation.phase ? " · " + escapeHtml(observation.phase) : ""}</span>
      </div>
      <div class="raw-inspector-sources">
        ${pos.length ? `<div class="raw-source-block observation"><span class="raw-source-label">Grün (lerntragend)</span><ul>${pos.map((i) => `<li>${escapeHtml(i.text)}</li>`).join("")}</ul></div>` : ""}
        ${dev.length ? `<div class="raw-source-block hint"><span class="raw-source-label">Blau (Entwicklungspotenzial)</span><ul>${dev.map((i) => `<li>${escapeHtml(i.text)}</li>`).join("")}</ul></div>` : ""}
        ${!pos.length && !dev.length ? `<p class="empty-state">Keine Items im Eintrag.</p>` : ""}
      </div>
    `;
  }
...
function renderViewShell() {
  ...
  mehrView?.classList.toggle("hidden", currentView !== "mehr");
  ...
  mehrTab?.classList.toggle("active", currentView === "mehr");
  ...
  detailPanel.querySelectorAll(".scale-button").forEach((button) => {
    button.addEventListener("click", (event) => {
      ...
      if (nextValue === 0) {
        ...
        spiderBatchChanges = spiderBatchChanges.filter((c) => c.key !== key);
      } else {
        const valence = nextValue <= 2 ? "positive" : "development";
        if (spiderSessionOpen) {
          const spiderItem = itemByKey(key);
          const itemText = spiderItem?.exactText ?? spiderItem?.text ?? spiderItem?.label ?? key;
          spiderBatchChanges = spiderBatchChanges.filter((c) => c.key !== key);
          spiderBatchChanges.push({ key, text: itemText, valence });
        } else {
...
function flushSpiderBatch() {
  if (!spiderBatchChanges.length) return;
  const positive = spiderBatchChanges.filter((c) => c.valence === "positive");
  const development = spiderBatchChanges.filter((c) => c.valence === "development");
  const obs = {
    id: "spider-batch-" + Date.now(),
    type: "spiderBatch",
    spiderBatch: true,
    text: "Spider-Web-Markierung",
    spiderBatchItems: spiderBatchChanges.map((c) => ({ key: c.key, text: c.text, valence: c.valence })),
    phase: livePhase?.value ?? "",
    socialForm: liveSocialFormValue,
    timestamp: new Date().toISOString(),
    manualValence: positive.length && !development.length ? "positive"
      : development.length && !positive.length ? "development" : null,
    analysisStatus: "dirty",
    lastAnalyzedAt: null,
    autoLinks: [],
    generatedCondensations: [],
    usedForInterpretation: true,
    greenItems: positive.map((c) => ({ key: c.key, text: c.text })),
    blueItems: development.map((c) => ({ key: c.key, text: c.text }))
  };
...
<head>
  ...
  <style>
    :root { --text:#162127; --muted:#5f6c73; --line:#d9e3e6; --surface:#ffffff; --bg:#f4f7f7; --green:#4a9f2f; --green-soft:#edf8e9; --blue:#2369a8; --blue-soft:#e7f2ff; --mixed:#2369a8; --shadow:0 18px 54px rgba(25,39,45,.10); font-family: Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; }
...
// Pure data. No logic here. Applied only during inactivity on dirty entries.
const LOCAL_RULES = [
  {
    id: "rule-leerlauf",
    keywords: ["leerlauf", "warten", "wartet", "nichts zu tun", "leer", "untätig"],
    autoLinkCategory: "Zeitnutzung",
    condensationHints: ["Leerlaufphase erkennbar", "Zeitverlust durch Wartezeit"]
  },
  {
    id: "rule-unruhe",
    keywords: ["unruhe", "laut", "lärm", "störung", "chaos", "reden", "quatschen"],
    autoLinkCategory: "Klassenführung",
    condensationHints: ["Unruhesituation", "Störung des Unterrichtsflusses"]
  },
  {
    id: "rule-lob",
    keywords: ["lob", "lobt", "toll", "super", "prima", "gut gemacht", "richtig", "genau", "bravo"],
    autoLinkCategory: "Feedback",
    condensationHints: ["Positive Rückmeldung durch LK", "Lob an SuS"]
  },
  {
    id: "rule-frage",
    keywords: ["fragt", "frage", "impuls", "anregung", "nachfrage", "warum", "wie könnte"],
    autoLinkCategory: "Aktivierung",
    condensationHints: ["Aktivierender Impuls", "Kognitive Herausforderung durch Frage"]
  },
  {
    id: "rule-schueleraeusserung",
    keywords: ["sus sagt", "schüler sagt", "schülerin sagt", "sus:", "s:", "l:", "lk:"],
    autoLinkCategory: "Beteiligung",
    condensationHints: ["Schüleräußerung dokumentiert", "Beteiligung einzelner SuS"]
  },
  {
    id: "rule-erklaerung",
    keywords: ["erklärt", "erklärung", "instruktion", "einführung", "einführt", "zeigt"],
    autoLinkCategory: "Strukturierung",
    condensationHints: ["Instruktionsphase", "LK erklärt Aufgabe oder Inhalt"]
  },
  {
    id: "rule-gruppenarbeit",
    keywords: ["gruppenarbeit", "gruppe", "partner", "partnerarbeit", "ga", "pa", "zusammenarbeit"],
    autoLinkCategory: "Sozialform",
    condensationHints: ["Kooperative Lernphase", "Partner- oder Gruppenarbeit aktiv"]
  },
  {
    id: "rule-differenzierung",
    keywords: ["differenzierung", "unterstützung", "hilfe", "hilft", "fördert", "einzeln", "individuell"],
    autoLinkCategory: "Individualisierung",
    condensationHints: ["Individuelle Unterstützung", "Differenzierungsmaßnahme erkennbar"]
  },
  {
    id: "rule-visualisierung",
    keywords: ["tafel", "folie", "beamer", "präsentation", "schreibt an", "zeigt auf", "visualisiert"],
    autoLinkCategory: "Medieneinsatz",
    condensationHints: ["Visualisierung eingesetzt", "Tafelbild oder Mediennutzung"]
  },
  {
    id: "rule-zeitstruktur",
    keywords: ["phase", "übergang", "wechsel", "jetzt", "als nächstes", "weiter", "abschluss"],
    autoLinkCategory: "Zeitstruktur",
    condensationHints: ["Phasenübergang", "Strukturierung des Zeitverlaufs"]
  },
  {
    id: "rule-rueckmeldung",
    keywords: ["korrektur", "fehler", "falsch", "korrigiert", "rückmeldung", "feedback", "hinweis gibt"],
    autoLinkCategory: "Feedback",
    condensationHints: ["Korrektur durch LK", "Inhaltliche Rückmeldung"]
  },
  {
    id: "rule-klima",
    keywords: ["respekt", "wertschätzung", "atmosphäre", "klima", "freundlich", "vertrauen", "beziehung"],
    autoLinkCategory: "Klassenklima",
    condensationHints: ["Positives Klassenklima erkennbar", "Respektvoller Umgang beobachtet"]
  }
];

function matchLocalRules(text) {
  if (!text || typeof text !== "string") return { autoLinks: [], generatedCondensations: [] };
  const lower = text.toLowerCase();
  const autoLinks = [];
  const generatedCondensations = [];
  const seen = new Set();
  for (const rule of LOCAL_RULES) {
    const hit = rule.keywords.some((kw) => lower.includes(kw));
    if (!hit || seen.has(rule.autoLinkCategory)) continue;
    seen.add(rule.autoLinkCategory);
    autoLinks.push({ ruleId: rule.id, category: rule.autoLinkCategory });
    if (rule.condensationHints.length) {
      generatedCondensations.push(rule.condensationHints[0]);
    }
  }
  return { autoLinks, generatedCondensations };
}
